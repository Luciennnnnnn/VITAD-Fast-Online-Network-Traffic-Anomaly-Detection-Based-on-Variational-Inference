...
def evaluate(dataset_name, ed, fraction, mu, sigma, R, SNR=None):
    # Run BayesCP
    Y, outliers_p = generator2(dataset_name, fraction, mu, sigma, SNR=SNR)
    if ed == None:
        ed = Y.shape[2]
    Y = Y[:, :, 0:ed]
    outliers_p = outliers_p[:, :, 0:ed]
    model = BCPF_IC(Y=Y, outliers_p=outliers_p, maxRank=R, maxiters=30, tol=1e-4, verbose=False, init=inits[dataset_name])
    return model
...
def eval_mu(dataset_name):
    ...
    mus2 = [0.01, 0.05]
    for mu in mus:
        model = evaluate(dataset_name, 8000, 0.1, mu, 0.1, Rs[dataset_name])
...
def eval_SNR(dataset_name):
    ...
    with open(os.path.join(os.path.join('../../results', dataset_name), 'proposed/SNR2_TPRS.json'), 'w') as FD:
        FD.write(json.dumps(TPRS))
    with open(os.path.join(os.path.join('../../results', dataset_name), 'proposed/SNR2_FPRS.json'), 'w') as FD:
        FD.write(json.dumps(FPRS))